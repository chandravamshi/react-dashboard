...
export const _appPatients = [...Array(7)].map((_, index) => ({
  id: `${Date.now() + index}`,
  name: _mock.name.fullName(index),
  termin: _mock.time(index),
  price: _mock.number.price(index) + 150,
  session: randomNumberRange(1, 15),
  package: randomInArray([{title:'Einzelsitzung', subtitle: null}, {title:'6 Sitzungen', subtitle: "(1 / 6)"}, {title:'6 Sitzungen', subtitle: "(3 / 6)"}, {title:'6 Sitzungen', subtitle: "(4 / 6)"}]),
  service: randomInArray(["PRP", "PRP + Meso", "Meso"]),
  status: randomInArray(['waiting', 'paid', 'unpaid', 'free', 'pays later']),
  totalRemaining: _mock.number.price(index) + 500,
  mesoSet: randomInArray([true, false, false]),
  customerType: randomInArray(["Neukunde", "Elit Bestandskunde", "PRP Bestandskunde"]),
}));
